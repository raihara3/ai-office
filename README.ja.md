...
- **カンバンとインボックスで人間がループに入る。** タスクカードを起票する
  ...
  セッションがあなたの回答待ちになったときや要確認の報告が届いたときは
  macOS 通知(デスクトップアプリではメニューバーのバッジも)が知らせ、
  人件費パネルで常駐AIごとのトークン使用量を確認できます。すべてローカル
  の SQLite ファイル 1 つに保存されます。
...
```sh
npm test          # node --test、約160件、ビルド不要
```
