...
- **`residents.js`** — オーケストレータ。30 秒ごとの tick で常駐行を
  再読込します。常駐員は `role` によって仕事の出所が分かれます。
  `board`(カンバン担当)はトリガーを無視し、アイドルなら自分のカンバン列の
  一番上のカードをタスクとして実行します(precheck はスキップし、カードの
  本文をプロンプトの「今回のタスク」節に含める)。`scheduled`(定期実行担当)
  はカンバンを見ず、期日が来たトリガーで自分の指示書(役割指示)をそのまま
  実行します。カードの割り当ては不要で、`precheck` シェルコマンドが設定
