...
// registry, runner and whiteboard into one handle for the core. A tick loop
// re-reads the manifests (so hand-edited files are picked up), fires due
// triggers, gates interval runs on their precheck command, and turns finished
// runs into whiteboard reports plus a #general notification.
...
function buildPrompt(configuration, instructions, precheckOutput) {
  const sections = [
...
export function createResidents({
  ...
  function handleFinished(entry, { outcome, resultText }) {
    ...
    whiteboard.saveReport(entry.name, {
      title: `${configuration.displayName} ${formatRunDate(finishedAt)}`,
      level: reportLevel,
      body: reportBody,
    ...
    state.postMessage({
      ...
      text:
        reportLevel === 'review-needed'
          ...
          : '@社長 作業が完了しました(ホワイトボードに報告を掲示しました)',
      at: finishedAt,
    });
    state.refresh();
  }
  ...
  // false for the panel's 今すぐ実行 button.
  async function launch(entry, { gateOnPrecheck }) {
    const { configuration } = entry;
    const startedAt = now();
    ...
    const started = runner.run(
      ...
      {
        prompt: buildPrompt(configuration, entry.instructions, precheckOutput),
        onFinished: (result) => handleFinished(entry, result),
      }
    );
  ...
  function tick() {
    for (const entry of manifestStore.list()) {
      ...
      if (runner.isRunning(entry.name)) continue;
      if (!isDue(entry.configuration.trigger, entry.state.lastRunAt ?? null, now())) continue;
